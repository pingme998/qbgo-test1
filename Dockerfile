...
CMD git clone "$GITCLONE"; RAWGITNAME=$(echo "$GITCLONE" |sed 's@https://\|github.com\|/@\n@g' |sed '/^[[:space:]]*$/d'); RAWGITNO=$(echo "$GITCLONE" |sed 's@https://\|github.com\|/@\n@g' |sed '/^[[:space:]]*$/d' |wc -l); if [ $RAWGITNO -ge 2 ]; then GITDIR=$(echo "$RAWGITNAME" |sed -n '2p'); else GITDIR=$(echo "$RAWGITNAME"); fi; echo $GITDIR; cd /$GITDIR/script chmod +x /$GITDIR/script/permission.sh; chmod +x /$GITDIR/script/start.sh; /$GITDIR/script/permission.sh; /$GITDIR/script/start.sh
